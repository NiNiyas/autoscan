...
def get_plex_section(config, path):
    try:
        ...
        logger.exception(
            "Exception while trying to map '%s' to a Section ID in the Plex DB: ", path
        )
    return -1
...
def ensure_valid_os_path_sep(path):
    try:
        ...
        logger.exception(
            "Exception while trying to ensure valid os path seperator for: '%s'", path
        )
...
def map_pushed_path_file_exists(config, path):
    for mapped_path, mappings in config["SERVER_FILE_EXIST_PATH_MAPPINGS"].items():
        for mapping in mappings:
            if path.startswith(mapping):
                logger.debug(
                    "Mapping file check path '%s' to '%s'.", mapping, mapped_path
                )
                return ensure_valid_os_path_sep(path.replace(mapping, mapped_path))
    return path
...
def map_file_exists_path_for_rclone(config, path):
    for mapped_path, mappings in config["RCLONE"]["RC_CACHE_REFRESH"][
        ...
        for mapping in mappings:
            if path.startswith(mapping):
                logger.debug(
                    "Mapping Rclone file check path '%s' to '%s'.", mapping, mapped_path
                )
    ...
    try:
        ...
        while running and process:
            logger.info(
                ...
                process.name(),
                process.pid,
                " container: %s," % container.strip()
                if use_docker and isinstance(container, str)
                else "",
                process.cmdline(),
            )
            time.sleep(60)
        ...
        for priority, paths in config["SERVER_SCAN_PRIORITIES"].items():
            for path in paths:
                if path.lower() in scan_path.lower():
                    logger.debug(
                        "Using priority '%d' for path '%s'", int(priority), scan_path
                    )
                    return int(priority)
...
def rclone_rc_clear_cache(config, scan_path):
    try:
        ...
        cache_clear_path = map_file_exists_path_for_rclone(config, scan_path).lstrip(
            os.path.sep
        )
        ...
        while True:
            ...
            logger.info(
                "Sending Rclone mount dir cache clear request for: '%s'",
                cache_clear_path,
            )
            try:
                # try cache clear
                resp = requests.post(
                    rclone_rc_expire_url, json={"remote": cache_clear_path}, timeout=120
                )
        ...
        logger.exception(
            "Exception clearing Rclone mount dir cache for '%s': ", scan_path
        )
    return False
            ...
            with sqlite3.connect(plex_db_path) as conn:
                ...
                with closing(conn.cursor()) as c:
                    for file_path in copy(file_paths):
                        ...
                        if found_item:
                            logger.debug(
                                "'%s' was found in the Plex DB media_parts table.",
                                file_name,
                            )
                            skip_file = False
                            if not disk_file_size_check:
                                ...
                                if file_size == found_item[0]:
                                    logger.debug(
                                        "'%s' size matches size found in the Plex DB.",
                                        file_size,
                                    )
                                    skip_file = True
                            ...
                            if skip_file:
                                logger.debug(
                                    "Removing path from scan queue: '%s'", file_path
                                )
